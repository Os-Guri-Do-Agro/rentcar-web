...
const TermsAndRules = () => {
  ...
  const scrollToSection = (id) => {
    ...
    if (element) {
      const offset = 120; // Height of header + nav
  ...
  return (
    <>
      ...
      {/* Hero Section */}
      <div className="bg-[#0E3A2F] text-white py-16 px-4">
              ...
              {regras.map((rule, index) => (
                ...
                >
                  <h3 className="text-xl font-bold text-[#0E3A2F] mb-4 flex items-center gap-2">
                    <span className="w-8 h-8 rounded-full bg-[#00D166]/20 flex items-center justify-center text-[#0E3A2F] text-sm">
                      {index + 1}
                    </span>
                    {rule.titulo}
                  </h3>
            ...
            <div className="grid gap-6">
              {termos.map((term, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: index * 0.05 }}
                  className="bg-white rounded-xl overflow-hidden shadow-md border border-gray-100 group hover:shadow-lg transition-all"
                >
                  <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                    <h3 className="font-bold text-lg text-[#0E3A2F]">{term.titulo}</h3>
                    <div className="bg-white px-3 py-1 rounded text-xs font-semibold text-gray-500 uppercase tracking-wider border border-gray-200">
                      {term.secao || 'Geral'}
                    </div>
                  </div>
