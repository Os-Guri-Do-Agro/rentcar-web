...
const PoliticaPrivacidade = () => {
    ...
    useEffect(() => {
        getConteudo('politica-privacidade').then(res => {
            setData(res);
            setLoading(false);
        });
    }, []);
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-12 px-4">
            <Helmet title={data?.titulo || "Política de Privacidade"} />
            <div className="max-w-4xl mx-auto bg-white p-8 md:p-12 rounded-2xl shadow-sm">
                <h1 className="text-3xl font-bold text-[#0E3A2F] mb-8 pb-4 border-b">{data?.titulo || "Política de Privacidade"}</h1>
                <div 
