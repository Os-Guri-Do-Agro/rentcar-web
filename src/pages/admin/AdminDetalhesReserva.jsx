...
import { Loader2, ArrowLeft, User, Mail, Phone, FileText, Car, CheckCircle, XCircle, Clock, Trash2 } from 'lucide-react';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from "@/components/ui/dialog";
import { confirmReserva, rejectReserva, deleteReserva, getReservaHistorico } from '@/services/reservaStatusService';
...
const AdminDetalhesReserva = () => {
    ...
    const [modals, setModals] = useState({ confirmReserva: false, rejectReserva: false, deleteReserva: false });
    ...
    const fetchData = async () => {
        try {
            const resData = await getReservaById(reservaId);
            setReserva(resData);
            const histData = await getReservaHistorico(reservaId);
    ...
    const handleAction = async (actionType) => {
        ...
        try {
            const actions = {
                confirmReserva: () => confirmReserva(reservaId, usuario.id),
                rejectReserva: () => rejectReserva(reservaId, reason, usuario.id),
                deleteReserva: () => deleteReserva(reservaId)
            };
            ...
            setModals({ confirmReserva: false, rejectReserva: false, deleteReserva: false });
            if (actionType === 'deleteReserva') navigate('/admin/reservas');
        } catch (error) {
        ...
        <div className="min-h-screen bg-gray-50 pb-20">
            ...
            <div className="bg-white border-b sticky top-0 z-30 px-6 py-4 flex items-center justify-between shadow-sm">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => setModals(p => ({...p, deleteReserva: true}))} className="flex items-center gap-2 bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-bold hover:bg-gray-300 text-sm"><Trash2 size={16} /> Excluir</button>
                </div>
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            ...
                            <div className="flex gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-lg text-gray-900">{reserva.cars?.marca} {reserva.cars?.nome}</h3>
                        ...
                        <div className="space-y-6 border-l border-gray-200 pl-6">
                            {historico.map((item) => (
                                <div key={item.id} className="relative">
                                    ...
                                    <p className="text-sm font-bold text-gray-800">{item.status_novo}</p>
            ...
            <Dialog open={modals.deleteReserva} onOpenChange={(open) => !open && setModals(p => ({...p, deleteReserva: false}))}><DialogContent><DialogHeader><DialogTitle>Excluir</DialogTitle><DialogDescription>Irreversível.</DialogDescription></DialogHeader><DialogFooter><button onClick={() => setModals(p => ({...p, deleteReserva: false}))} className="px-4 py-2 text-gray-600">Voltar</button><button onClick={() => handleAction('deleteReserva')} disabled={processing} className="px-4 py-2 bg-gray-600 text-white rounded font-bold">{processing && <Loader2 className="animate-spin" size={16}/>} Excluir</button></DialogFooter></DialogContent></Dialog>
        </div>
    );
};
